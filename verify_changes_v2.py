...
def run(playwright):
    ...
    page.fill('input[type="tel"]', "+917002492207")
    ...
    mobile_page.fill('input[type="tel"]', "+917002492207")
    ...
    # In CSS we use transform to hide it, so it might be "visible" in DOM but off-screen.
    # Let's check the class.
    # We expect .sidebar to NOT have .sidebar-open initially
    if "sidebar-open" in sidebar.get_attribute("class"):
    ...
    if garden_select.is_visible():
        garden_select.select_option(index=1) # Select first option after default
